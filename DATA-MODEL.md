...
aliases/{aliasId}
  ...
  suggestions/{suggestionId}             // история для автозаполнения
    - name, nameLower, count, lastUsedAt
                                          // v1.2: удаление подсказки = удаление документа
                                          //   (нет флага hidden). recordSuggestion (setDoc
                                          //   merge) пересоздаёт её при повторном добавлении
                                          //   позиции — поэтому удалённая подсказка
                                          //   возвращается, если снова добавить такой товар.
```
